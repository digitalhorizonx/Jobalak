...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (ref) {
      window.localStorage.setItem("jobalak:referrer", ref.slice(0, 80));
      setReferred(true);
    }
  }, []);
  ...
  function validateRequest() {
    ...
    if (selected.length === 0) return "اختار دولة واحدة على الأقل.";
    return "";
  }

  function persistRequest() {
    window.localStorage.setItem("jobalak:lastRequest", JSON.stringify({
      email: email.trim().toLowerCase(),
      countries: selected,
      visa,
      fileName,
      referrer: window.localStorage.getItem("jobalak:referrer") || "",
