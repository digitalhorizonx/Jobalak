...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(PAYPAL_API + "/v2/checkout/orders", {
      ...
      body: JSON.stringify({
        intent: "CAPTURE",
        purchase_units: [{ description: "Jobalak AI Job Search", amount: { currency_code: "USD", value: "1.50" } }],
        application_context: {
          brand_name: "Jobalak",
          user_action: "PAY_NOW",
    ...
    const safeError = code === "missing_paypal_credentials" || code === "paypal_auth_failed" ? code : "paypal_unavailable";
